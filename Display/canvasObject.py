...
class CanvasObject(Screen, metaclass=ABCMeta):
    ...
    def clear(self):
        ...
        if self.usePygame:
            ...
            self.canvas.create_rectangle(0, 0, self.window.width, self.window.height, fill = "#%02x%02x%02x" % self.backgroundColor)
    ...
    def showRectangle(self, position1, position2, color, secondaryColor=(0,0,0), width=0, shiftPosition=False):
        ...
        if x1 > self.window.width:
            show = False
        if x2 < 0:
            show = False
        if y1 > self.window.height:
            show = False
        if y2 < 0:
            show = False
    ...
    def showLine(self, position1, position2, color, width, shiftPosition=False):
        ...
        if x1 < self.window.width and x1 > 0:
            if y1 < self.window.height and y1 > 0:
                show = True
        if x2 < self.window.width and x2 > 0:
            if y2 < self.window.height and y2 > 0:
                show = True
    ...
    def showPolygon(self, pointList, color, position=(0, 0), shiftPosition=False):
        ...
        for point in pointList:
            if not show:
                if point[0] < self.window.width and point[0] > 0:
                    if point[1] < self.window.height and point[1] > 0:
                        show = True
        if show:
            if self.usePygame:
    ...
    def showCircle(self, position, radius, color, width=0, secondaryColor=(0, 0, 0), shiftPosition=False):
        ...
        show = True
        if x - radius / 2 > self.window.width:
            show = False
        if x + radius / 2 < 0:
            show = False
        if y - radius / 2 > self.window.height:
            show = False
        if y + radius / 2 < 0:
            show = False
